...
# add YEAR column to data as first 4 chars of DATE, we'll use this to partition on  
df1 = df.withColumn('YEAR', df['DATE'].substr(0, 4))

# partition the DataFrame to 1 partition
partitioned_dataframe = df1.repartition(1)       

# convert the DataFrame to a DynamicFrame so we can use Glue functions
partitioned_dynamicframe = DynamicFrame.fromDF(partitioned_dataframe, glueContext, "partitioned_df")

# drop the null columns, Parquet can't handle a column that is all null so this is necessary for years <= 2016, when it seems NUTS codes didn't exist
## @type: DropNullFields
